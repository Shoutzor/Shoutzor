...
class getid3_id3v1 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (substr($id3v1tag, 0, 3) == 'TAG') {
            ...
            $ParsedID3v1['comment'] =
                substr($id3v1tag, 97, 30);  // can't remove nulls yet, track detection depends on them
                ...
                foreach ($valuearray as $key => $value) {
                    if (preg_match('#^[\\x00-\\x40\\xA8\\xB8\\x80-\\xFF]+$#', $value)) {
                        foreach (array('Windows-1251', 'KOI8-R') as $id3v1_bad_encoding) {
                            if (function_exists('mb_convert_encoding') && @mb_convert_encoding(
                                    $value,
                                    $id3v1_bad_encoding,
                                    $id3v1_bad_encoding
                                ) === $value) {
                                ...
                                break 3;
                            } elseif (function_exists('iconv') && @iconv(
                                    $id3v1_bad_encoding,
                                    $id3v1_bad_encoding,
                                    $value
                                ) === $value) {
