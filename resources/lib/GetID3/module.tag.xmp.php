        ...
        for ($i = 0; $i < count($jpeg_header_data); $i++) {
            ...
            if (strcmp($jpeg_header_data[$i]['SegName'], 'APP1') == 0) {
                ...
                if (strncmp($jpeg_header_data[$i]['SegData'], 'http://ns.adobe.com/xap/1.0/' . "\x00", 29) == 0) {
                    ...
                    return trim(
                        $xmp_data
                    ); // trim() should not be neccesary, but some files found in the wild with null-terminated block (known samples from Apple Aperture) causes problems elsewhere (see https://www.getid3.org/phpBB3/viewtopic.php?f=4&t=1153)
                }
            }
        }
        return false;
            ...
            switch ($xml_elem['tag']) {
                ...
                case 'rdf:li':
                    ...
                    if ($xml_elem['type'] == 'complete') {
                        ...
                        if ($current_property != '') {
                            $xmp_array[$current_property][$container_index] =
                                (isset($xml_elem['value']) ? $xml_elem['value'] : '');
                            $container_index += 1;
                        }
                    ...
                    if (true) {
                        switch ($xml_elem['type']) {
                            ...
                            case 'complete':
                                ...
                                $xmp_array[$xml_elem['tag']] =
                                    (isset($xml_elem['attributes']) ? $xml_elem['attributes'] : (isset($xml_elem['value']) ? $xml_elem['value'] : ''));
                                break;
