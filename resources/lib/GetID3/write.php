...
class getid3_writetags
{
        ...
        if (filesize($this->filename) == 0) {
            ...
            foreach ($this->tagformats as $requested_tag_format) {
                if (!in_array($requested_tag_format, $AllowedTagFormats)) {
                    $errormessage =
                        'Tag format "' . $requested_tag_format . '" is not allowed on "' . (isset($this->ThisFileInfo['fileformat']) ? $this->ThisFileInfo['fileformat'] : '');
            ...
            if ($this->remove_other_tags) {
                foreach ($AllowedTagFormats as $AllowedTagFormat) {
                    switch ($AllowedTagFormat) {
                        ...
                        case 'id3v2.3':
                        case 'id3v2.4':
                            if (!in_array('id3v2', $TagFormatsToRemove) && !in_array(
                                    'id3v2.2',
                                    $this->tagformats
                                ) && !in_array('id3v2.3', $this->tagformats) && !in_array(
                                    'id3v2.4',
                                    $this->tagformats
                                )) {
        ...
        if (!empty($this->ThisFileInfo['tags'])) {
            foreach ($this->ThisFileInfo['tags'] as $tag_format => $tag_data_array) {
                foreach ($tag_data_array as $tag_key => $tag_array) {
                    if (strtoupper($tag_key) !== $tag_key) {
                        $this->ThisFileInfo['tags'][$tag_format][strtoupper($tag_key)] =
            ...
            switch ($DeleteTagFormat) {
                case 'id3v1':
                    ...
                    if (($success = $id3v1_writer->RemoveID3v1()) === false) {
                        $this->errors[] = 'RemoveID3v1() failed with message(s):<PRE><UL><LI>' . trim(
                                implode('</LI><LI>', $id3v1_writer->errors)
                            ) . '</LI></UL></PRE>';
                    }
                    break;
                ...
                case 'id3v2':
                    ...
                    if (($success = $id3v2_writer->RemoveID3v2()) === false) {
                        $this->errors[] = 'RemoveID3v2() failed with message(s):<PRE><UL><LI>' . trim(
                                implode('</LI><LI>', $id3v2_writer->errors)
                            ) . '</LI></UL></PRE>';
                    }
                    break;
                ...
                case 'ape':
                    ...
                    if (($success = $ape_writer->DeleteAPEtag()) === false) {
                        $this->errors[] = 'DeleteAPEtag() failed with message(s):<PRE><UL><LI>' . trim(
                                implode('</LI><LI>', $ape_writer->errors)
                            ) . '</LI></UL></PRE>';
                    }
                    break;
                ...
                case 'vorbiscomment':
                    ...
                    if (($success = $vorbiscomment_writer->DeleteVorbisComment()) === false) {
                        $this->errors[] = 'DeleteVorbisComment() failed with message(s):<PRE><UL><LI>' . trim(
                                implode('</LI><LI>', $vorbiscomment_writer->errors)
                            ) . '</LI></UL></PRE>';
                    }
                    break;
                ...
                case 'metaflac':
                    ...
                    if (($success = $metaflac_writer->DeleteMetaFLAC()) === false) {
                        $this->errors[] = 'DeleteMetaFLAC() failed with message(s):<PRE><UL><LI>' . trim(
                                implode('</LI><LI>', $metaflac_writer->errors)
                            ) . '</LI></UL></PRE>';
                    }
                    break;
                ...
                case 'lyrics3':
                    ...
                    if (($success = $lyrics3_writer->DeleteLyrics3()) === false) {
                        $this->errors[] = 'DeleteLyrics3() failed with message(s):<PRE><UL><LI>' . trim(
                                implode('</LI><LI>', $lyrics3_writer->errors)
                            ) . '</LI></UL></PRE>';
                    }
                    break;
                ...
                case 'real':
                    ...
                    if (($success = $real_writer->RemoveReal()) === false) {
                        $this->errors[] = 'RemoveReal() failed with message(s):<PRE><UL><LI>' . trim(
                                implode('</LI><LI>', $real_writer->errors)
                            ) . '</LI></UL></PRE>';
                    }
                    break;
                ...
                default:
                    foreach ($valuearray as $key => $value) {
                        if (is_string($value) || is_numeric($value)) {
                            $ape_tag_data[$tag_key][$key] =
                                getid3_lib::iconv_fallback($this->tag_encoding, 'UTF-8', $value);
                        } else {
                            $this->warnings[] =
                                '$data[' . $tag_key . '][' . $key . '] is not a string value - all of $data[' . $tag_key . '] NOT written to APE tag';
    ...
    public function FormatDataForID3v1()
    {
        ...
        $tag_data_id3v1['title'] = getid3_lib::iconv_fallback(
            $this->tag_encoding,
            'ISO-8859-1',
            implode(' ', (isset($this->tag_data['TITLE']) ? $this->tag_data['TITLE'] : array()))
        );
        $tag_data_id3v1['artist'] = getid3_lib::iconv_fallback(
            $this->tag_encoding,
            'ISO-8859-1',
            implode(' ', (isset($this->tag_data['ARTIST']) ? $this->tag_data['ARTIST'] : array()))
        );
        $tag_data_id3v1['album'] = getid3_lib::iconv_fallback(
            $this->tag_encoding,
            'ISO-8859-1',
            implode(' ', (isset($this->tag_data['ALBUM']) ? $this->tag_data['ALBUM'] : array()))
        );
        $tag_data_id3v1['year'] = getid3_lib::iconv_fallback(
            $this->tag_encoding,
            'ISO-8859-1',
            implode(' ', (isset($this->tag_data['YEAR']) ? $this->tag_data['YEAR'] : array()))
        );
        $tag_data_id3v1['comment'] = getid3_lib::iconv_fallback(
            $this->tag_encoding,
            'ISO-8859-1',
            implode(' ', (isset($this->tag_data['COMMENT']) ? $this->tag_data['COMMENT'] : array()))
        );
        $tag_data_id3v1['track_number'] = intval(
            getid3_lib::iconv_fallback(
                $this->tag_encoding,
                'ISO-8859-1',
                implode(
                    ' ',
                    (isset($this->tag_data['TRACK_NUMBER']) ? $this->tag_data['TRACK_NUMBER'] : array())
                )
            )
        );
    ...
    public function FormatDataForID3v2($id3v2_majorversion)
    {
        ...
        foreach ($this->tag_data as $tag_key => $valuearray) {
            ...
            switch ($ID3v2_framename) {
                ...
                case 'POPM':
                    if (isset($valuearray['email']) && isset($valuearray['rating']) && isset($valuearray['data'])) {
                ...
                case 'GRID':
                    if (isset($valuearray['groupsymbol']) && isset($valuearray['ownerid']) && isset($valuearray['data'])) {
                ...
                case 'UFID':
                    if (isset($valuearray['ownerid']) && isset($valuearray['data'])) {
                ...
                case '':
                    $this->errors[] =
                        'ID3v2: Skipping "' . $tag_key . '" because cannot match it to a known ID3v2 frame type';
                ...
                default:
                    ...
                    foreach ($valuearray as $key => $value) {
                        if (isset($ID3v2_text_encoding_lookup[$id3v2_majorversion][$this->tag_encoding])) {
                            ...
                            $tag_data_id3v2[$ID3v2_framename][$key]['encodingid'] =
                            ...
                            if ($id3v2_majorversion < 4) {
                                ...
                                if (!$ID3v2_tag_data_converted) {
                                    ...
                                    $tag_data_id3v2[$ID3v2_framename][$key]['data'] =
                                        "\xFF\xFE" . getid3_lib::iconv_fallback(
                                            $this->tag_encoding,
                                            'UTF-16LE',
                                            $value
            ...
            foreach ($valuearray as $key => $value) {
                if (($tag_key == 'ATTACHED_PICTURE') && is_array($value)) {
                    ...
                    if (strstr($value, "\n")) {
                        ...
                        foreach ($multilineexploded as $newcomment) {
                            if (strlen(trim($newcomment)) > 0) {
                                $tag_data_vorbiscomment[$tag_key][] =
                                    getid3_lib::iconv_fallback($this->tag_encoding, 'UTF-8', $newcomment);
                            }
                        }
                        ...
                        $this->warnings[] =
                            '$data[' . $tag_key . '][' . $key . '] is not a string value - all of $data[' . $tag_key . '] NOT written to VorbisComment tag';
    ...
    {
        $tag_data_real['title'] = getid3_lib::iconv_fallback(
            $this->tag_encoding,
            'ISO-8859-1',
            implode(' ', (isset($this->tag_data['TITLE']) ? $this->tag_data['TITLE'] : array()))
        );
        $tag_data_real['artist'] = getid3_lib::iconv_fallback(
            $this->tag_encoding,
            'ISO-8859-1',
            implode(' ', (isset($this->tag_data['ARTIST']) ? $this->tag_data['ARTIST'] : array()))
        );
        $tag_data_real['copyright'] = getid3_lib::iconv_fallback(
            $this->tag_encoding,
            'ISO-8859-1',
            implode(' ', (isset($this->tag_data['COPYRIGHT']) ? $this->tag_data['COPYRIGHT'] : array()))
        );
        $tag_data_real['comment'] = getid3_lib::iconv_fallback(
            $this->tag_encoding,
            'ISO-8859-1',
            implode(' ', (isset($this->tag_data['COMMENT']) ? $this->tag_data['COMMENT'] : array()))
        );
