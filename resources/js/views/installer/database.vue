<template>
    <div class="card card-md">
        <div class="card-body bg-white py-4 p-sm-4">
                ...
                <div>
                    ...
                    <div>
                        <div class="mb-3">
                            ...
                            <select v-model="dbType" class="form-select" name="dbtype">
                        ...
                        <div v-for="(properties, field) in activeDbFields" class="mb-3">
                            <div class="form-label">{{ $filters.capitalize(field) }}</div>
                            <input
                                v-model="dbFieldsData[field]"
                                :name="field"
                                :type="properties.type"
                                :placeholder="properties.default"
                                :class="(field in messages ? 'is-invalid' : '')"
                                class="form-control" />
                        </div>
                    </div>
                </div>
            ...
            <div v-else-if="hasConnection === true" class="alert alert-success d-flex align-items-center">
                <b-icon-check2-square class="flex-shrink-0 me-2" height="2rem" width="2rem"></b-icon-check2-square>
                <p>Connection success</p>
            </div>

            <div v-else class="d-flex justify-content-center pb-2">
    ...
    methods: {
        ...
        testSettings() {
            ...
            axios.post('/installer/database/configureDatabase', {
                dbtype: this.dbType, ...this.dbFieldsData
            }).then(res => {
